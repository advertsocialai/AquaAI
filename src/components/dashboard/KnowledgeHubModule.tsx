...
function ItemCard({ item }: { item: Item }) {
  ...
  return (
    <motion.div
      whileHover={{ y: -2 }}
      className="p-4 rounded-xl border border-border bg-card hover:bg-muted cursor-pointer group transition"
    >
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {item.tag && (
          <span className="px-2 py-0.5 rounded-full text-[10px] border border-border bg-card text-foreground/60">
            {item.tag}
          </span>
        )}
        {item.lang?.map((l) => (
...
export function KnowledgeHubModule() {
  ...
  return (
    <div className="space-y-6">
      ...
      {items.length === 0 ? (
        <div className="text-center py-12 text-sm text-foreground/30">No matches for "{query}"</div>
      ) : (
