  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {COMPLIANCE.map((c) => (
          <div key={c.metric} className="p-4 rounded-xl border border-border bg-card">
            <Shield className="w-4 h-4 mb-2 text-red-400" />
            <div className="text-2xl font-bold text-foreground tabular-nums">{c.value}</div>
            <div className="text-xs text-foreground/40">{c.metric}</div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-[11px] uppercase tracking-widest text-foreground/30">Active outbreak clusters</span>
          <span className="ml-auto text-[10px] text-foreground/30">Last 14 days · NSPAAD sync</span>
        </div>
        <div className="overflow-x-auto rounded-2xl border border-border">
          <table className="w-full text-sm">
            ...
            <tbody>
              {OUTBREAKS.map((o, i) => (
                <motion.tr key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="border-b border-border hover:bg-muted">
                  <td className="px-4 py-3 text-foreground/90 flex items-center gap-2"><MapPin className="w-3 h-3 text-red-400" /> {o.district}</td>
                  <td className="px-4 py-3 text-foreground/50 text-xs">{o.state}</td>
                  <td className="px-4 py-3 text-foreground/70 text-xs">{o.species}</td>
                  <td className="px-4 py-3 font-mono text-xs text-red-300">{o.disease}</td>
                  <td className="px-4 py-3 text-foreground/90 tabular-nums">{o.farms}</td>
                  <td className="px-4 py-3"><SeverityChip severity={o.severity} /></td>
                  <td className="px-4 py-3"><button className="text-xs text-foreground/40 hover:text-foreground inline-flex items-center gap-1"><Eye className="w-3 h-3" /> View</button></td>
                </motion.tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <div className="p-5 rounded-2xl border border-border bg-card">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-sm font-semibold text-foreground">Compliance Reports</span>
          </div>
          <ul className="space-y-2 text-sm">
            {[
              ...
              <li key={r.label} className="flex items-center justify-between p-2 rounded-lg bg-card">
                <span className="text-foreground/80 text-xs">{r.label}</span>
        ...
        <div className="p-5 rounded-2xl border border-border bg-card">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-sm font-semibold text-foreground">PMMSY Scheme Reach</span>
          </div>
          <div className="space-y-2 text-sm">
            <div className="flex items-center justify-between"><span className="text-foreground/70">Beneficiaries onboarded</span><span className="font-bold text-foreground tabular-nums">42,108</span></div>
            <div className="flex items-center justify-between"><span className="text-foreground/70">Subsidies disbursed</span><span className="font-bold text-emerald-400">₹187.3 Cr</span></div>
            <div className="flex items-center justify-between"><span className="text-foreground/70">Aerator subsidy claims</span><span className="font-bold text-foreground tabular-nums">3,891</span></div>
            <div className="flex items-center justify-between"><span className="text-foreground/70">Pond construction grants</span><span className="font-bold text-foreground tabular-nums">612</span></div>
          </div>
        </div>
      </div>
