...
function ItemCard({ item, accent }: { item: Item; accent: string }) {
  return (
    <motion.div
      whileHover={{ y: -2 }}
      className="p-4 rounded-xl border border-white/10 bg-white/[0.03] hover:bg-white/[0.06] transition group"
    >
      <div className="flex items-start justify-between gap-3 mb-2">
        <div className="flex-1 min-w-0">
          <div className="text-sm font-semibold text-white truncate">{item.name}</div>
          <div className="text-xs text-white/40 truncate">{item.spec}</div>
        </div>
      ...
      <div className="flex items-baseline justify-between mt-3">
        <div>
          ...
          {item.unit && <div className="text-[10px] text-white/30">{item.unit}</div>}
        </div>
        <button className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-white/10 bg-white/5 hover:bg-white/10 transition text-xs text-white/70 opacity-0 group-hover:opacity-100">
