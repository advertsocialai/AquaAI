  ...
  return (
    <div className="w-full grid lg:grid-cols-2 gap-6">
      ...
      <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-white/8 to-white/3 p-6">
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="shrink-0 text-center">
            ...
            <div className="text-[9px] text-white/40 mt-1 flex items-center justify-center gap-1">
      ...
      <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-lg font-bold text-white">Verify a Certificate</h3>
            <p className="text-xs text-white/50">Public verification — no login needed</p>
          </div>
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 rounded-lg bg-white/5 border border-white/10 px-3 py-2 text-sm text-white
                       placeholder:text-white/30 focus:outline-none focus:border-emerald-400/50 transition"
          />
          <button
            onClick={verify}
            disabled={status === 'loading'}
            className="px-4 py-2 rounded-lg bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50
                       text-white text-sm font-semibold transition flex items-center gap-1.5"
          >
...
function CertRow({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="flex justify-between gap-3 text-sm">
      ...
      <span className={`text-white/85 text-right ${mono ? 'font-mono text-xs' : ''}`}>
        {value}
      </span>
    </div>
  );
